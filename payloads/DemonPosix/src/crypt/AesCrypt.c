/*
 * AesCrypt.c — AES-256 CBC encryption / decryption (no external library)
 *
 * Pure-C implementation of AES-256 CBC so the agent has zero crypto
 * dependencies at compile time. The implementation is a standard,
 * well-known AES reference implementation.
 *
 * The interface matches the usage in the Windows Demon:
 *   - Encrypt / Decrypt operate on whole blocks (PKCS#7 padding).
 *   - Key = 32 bytes, IV = 16 bytes.
 */

#include "DemonPosix.h"

/* ── AES S-box and inverse S-box ─────────────────────────────────────── */
static const uint8_t sbox[256] = {
    
};

static const uint8_t rsbox[256] = {
    
};

static const uint8_t Rcon[11] = {
    0x8d,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

#define ROTL8(x,shift) ((uint8_t)(((x)<<(shift)) | ((x)>>(8-(shift)))))

static uint8_t xtime(uint8_t x) { return ((x<<1) ^ (((x>>7) & 1) * 0x1b)); }
static uint8_t Multiply(uint8_t x, uint8_t y) {
    return (((y & 1) * x) ^
            ((y>>1 & 1) * xtime(x)) ^
            ((y>>2 & 1) * xtime(xtime(x))) ^
            ((y>>3 & 1) * xtime(xtime(xtime(x)))) ^
            ((y>>4 & 1) * xtime(xtime(xtime(xtime(x))))));
}

/* ── Key expansion ───────────────────────────────────────────────────── */
typedef struct { uint8_t RoundKey[240]; } AES_CTX;

static void KeyExpansion(AES_CTX *ctx, const uint8_t *key)
{
    uint8_t tempa[4];
    int i, j, k;
    for (i = 0; i < 8; i++) {
        ctx->RoundKey[(i*4)+0] = key[(i*4)+0];
        ctx->RoundKey[(i*4)+1] = key[(i*4)+1];
        ctx->RoundKey[(i*4)+2] = key[(i*4)+2];
        ctx->RoundKey[(i*4)+3] = key[(i*4)+3];
    }
    for (; i < 60; i++) {
        k = (i-1) * 4;
        tempa[0] = ctx->RoundKey[k+0];
        tempa[1] = ctx->RoundKey[k+1];
        tempa[2] = ctx->RoundKey[k+2];
        tempa[3] = ctx->RoundKey[k+3];
        if (i % 8 == 0) {
            uint8_t u8tmp = tempa[0];
            tempa[0] = sbox[tempa[1]] ^ Rcon[i/8];
            tempa[1] = sbox[tempa[2]];
            tempa[2] = sbox[tempa[3]];
            tempa[3] = sbox[u8tmp];
        } else if (i % 8 == 4) {
            tempa[0] = sbox[tempa[0]];
            tempa[1] = sbox[tempa[1]];
            tempa[2] = sbox[tempa[2]];
            tempa[3] = sbox[tempa[3]];
        }
        j = i*4; k = (i-8)*4;
        ctx->RoundKey[j+0] = ctx->RoundKey[k+0] ^ tempa[0];
        ctx->RoundKey[j+1] = ctx->RoundKey[k+1] ^ tempa[1];
        ctx->RoundKey[j+2] = ctx->RoundKey[k+2] ^ tempa[2];
        ctx->RoundKey[j+3] = ctx->RoundKey[k+3] ^ tempa[3];
    }
}

/* ── Single block encrypt (in-place, state[4][4]) ────────────────────── */
typedef uint8_t state_t[4][4];

static void AddRoundKey(uint8_t round, state_t *state, const uint8_t *rk)
{
    for (int i=0;i<4;i++)
        for (int j=0;j<4;j++)
            (*state)[i][j] ^= rk[round*16 + i*4 + j];
}
static void SubBytes(state_t *s) {
    for (int i=0;i<4;i++) for(int j=0;j<4;j++) (*s)[i][j]=sbox[(*s)[i][j]];
}
static void ShiftRows(state_t *s) {
    uint8_t tmp;
    tmp=(*s)[0][1]; (*s)[0][1]=(*s)[1][1]; (*s)[1][1]=(*s)[2][1]; (*s)[2][1]=(*s)[3][1]; (*s)[3][1]=tmp;
    tmp=(*s)[0][2]; (*s)[0][2]=(*s)[2][2]; (*s)[2][2]=tmp; tmp=(*s)[1][2]; (*s)[1][2]=(*s)[3][2]; (*s)[3][2]=tmp;
    tmp=(*s)[3][3]; (*s)[3][3]=(*s)[2][3]; (*s)[2][3]=(*s)[1][3]; (*s)[1][3]=(*s)[0][3]; (*s)[0][3]=tmp;
}
static void MixColumns(state_t *s) {
    for (int i=0;i<4;i++) {
        uint8_t t=(*s)[i][0], tmp=(*s)[i][0]^(*s)[i][1]^(*s)[i][2]^(*s)[i][3];
        (*s)[i][0]^=xtime((*s)[i][0]^(*s)[i][1])^tmp;
        (*s)[i][1]^=xtime((*s)[i][1]^(*s)[i][2])^tmp;
        (*s)[i][2]^=xtime((*s)[i][2]^(*s)[i][3])^tmp;
        (*s)[i][3]^=xtime((*s)[i][3]^t)^tmp;
    }
}
static void InvSubBytes(state_t *s){ for(int i=0;i<4;i++)for(int j=0;j<4;j++)(*s)[i][j]=rsbox[(*s)[i][j]]; }
static void InvShiftRows(state_t *s){
    uint8_t tmp;
    tmp=(*s)[3][1];(*s)[3][1]=(*s)[2][1];(*s)[2][1]=(*s)[1][1];(*s)[1][1]=(*s)[0][1];(*s)[0][1]=tmp;
    tmp=(*s)[0][2];(*s)[0][2]=(*s)[2][2];(*s)[2][2]=tmp;tmp=(*s)[1][2];(*s)[1][2]=(*s)[3][2];(*s)[3][2]=tmp;
    tmp=(*s)[0][3];(*s)[0][3]=(*s)[1][3];(*s)[1][3]=(*s)[2][3];(*s)[2][3]=(*s)[3][3];(*s)[3][3]=tmp;
}
static void InvMixColumns(state_t *s){
    for(int i=0;i<4;i++){
        uint8_t a=(*s)[i][0],b=(*s)[i][1],c=(*s)[i][2],d=(*s)[i][3];
        (*s)[i][0]=Multiply(a,0x0e)^Multiply(b,0x0b)^Multiply(c,0x0d)^Multiply(d,0x09);
        (*s)[i][1]=Multiply(a,0x09)^Multiply(b,0x0e)^Multiply(c,0x0b)^Multiply(d,0x0d);
        (*s)[i][2]=Multiply(a,0x0d)^Multiply(b,0x09)^Multiply(c,0x0e)^Multiply(d,0x0b);
        (*s)[i][3]=Multiply(a,0x0b)^Multiply(b,0x0d)^Multiply(c,0x09)^Multiply(d,0x0e);
    }
}

static void Cipher(state_t *s, const uint8_t *rk)
{
    AddRoundKey(0, s, rk);
    for (int round=1; round<14; round++) {
        SubBytes(s); ShiftRows(s); MixColumns(s); AddRoundKey(round, s, rk);
    }
    SubBytes(s); ShiftRows(s); AddRoundKey(14, s, rk);
}
static void InvCipher(state_t *s, const uint8_t *rk)
{
    AddRoundKey(14, s, rk);
    for (int round=13; round>0; round--) {
        InvShiftRows(s); InvSubBytes(s); AddRoundKey(round, s, rk); InvMixColumns(s);
    }
    InvShiftRows(s); InvSubBytes(s); AddRoundKey(0, s, rk);
}

/* Copy bytes into state[col][row] layout */
static void buf_to_state(state_t *s, const uint8_t *b)
{
    for (int i=0;i<4;i++) for(int j=0;j<4;j++) (*s)[i][j]=b[i*4+j];
}
static void state_to_buf(uint8_t *b, const state_t *s)
{
    for (int i=0;i<4;i++) for(int j=0;j<4;j++) b[i*4+j]=(*s)[i][j];
}

/* ── Public API ─────────────────────────────────────────────────────── */

/*
 * AES256_Encrypt — AES-256 CBC encrypt with PKCS#7 padding.
 * out must be at least in_len + AES_BLOCK_SIZE bytes.
 * Returns 0 on success, -1 on error.
 */
int AES256_Encrypt(const uint8_t *in,  size_t in_len,
                   uint8_t       *out, size_t *out_len,
                   const uint8_t *key, const uint8_t *iv)
{
    if (!in || !out || !out_len || !key || !iv) return -1;

    AES_CTX ctx;
    KeyExpansion(&ctx, key);

    /* PKCS#7 padding */
    size_t pad_len   = AES_BLOCK_SIZE - (in_len % AES_BLOCK_SIZE);
    size_t total_len = in_len + pad_len;
    *out_len = total_len;

    uint8_t *buf = (uint8_t *)malloc(total_len);
    if (!buf) return -1;
    memcpy(buf, in, in_len);
    memset(buf + in_len, (uint8_t)pad_len, pad_len);

    uint8_t prev[AES_BLOCK_SIZE];
    memcpy(prev, iv, AES_BLOCK_SIZE);

    for (size_t i = 0; i < total_len; i += AES_BLOCK_SIZE) {
        for (int j = 0; j < AES_BLOCK_SIZE; j++)
            buf[i+j] ^= prev[j];
        state_t s;
        buf_to_state(&s, buf+i);
        Cipher(&s, ctx.RoundKey);
        state_to_buf(out+i, &s);
        memcpy(prev, out+i, AES_BLOCK_SIZE);
    }

    free(buf);
    return 0;
}

/*
 * AES256_Decrypt — AES-256 CBC decrypt, removes PKCS#7 padding.
 * out must be at least in_len bytes.
 * Returns 0 on success, -1 on error.
 */
int AES256_Decrypt(const uint8_t *in,  size_t in_len,
                   uint8_t       *out, size_t *out_len,
                   const uint8_t *key, const uint8_t *iv)
{
    if (!in || !out || !out_len || !key || !iv) return -1;
    if (in_len == 0 || (in_len % AES_BLOCK_SIZE) != 0) return -1;

    AES_CTX ctx;
    KeyExpansion(&ctx, key);

    uint8_t prev[AES_BLOCK_SIZE], tmp[AES_BLOCK_SIZE];
    memcpy(prev, iv, AES_BLOCK_SIZE);

    for (size_t i = 0; i < in_len; i += AES_BLOCK_SIZE) {
        memcpy(tmp, in+i, AES_BLOCK_SIZE);
        state_t s;
        buf_to_state(&s, in+i);
        InvCipher(&s, ctx.RoundKey);
        state_to_buf(out+i, &s);
        for (int j = 0; j < AES_BLOCK_SIZE; j++)
            out[i+j] ^= prev[j];
        memcpy(prev, tmp, AES_BLOCK_SIZE);
    }

    /* Remove PKCS#7 padding */
    uint8_t pad = out[in_len-1];
    if (pad == 0 || pad > AES_BLOCK_SIZE) return -1;
    *out_len = in_len - pad;

    return 0;
}
