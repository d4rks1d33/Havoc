...
void HavocNamespace::UserInterface::Widgets::TeamserverTabSession::setupUi( QWidget* Page, QString TeamserverName )
{
    ...
    // Set initial splitter proportions so panels are visible on first open.
    // Without this, Qt on macOS initialises both handles at 0 and the layout
    // looks completely blank until the user manually drags the splitters.
    //
    // splitter_TopBot (vertical):     top ~60 %  / bottom ~40 %
    // splitter_SessionAndTabs (horiz): left ~75 % / right ~25 %
    //
    // We use QTimer::singleShot so the sizes are applied after the widget has
    // been shown and its geometry is fully known.
    QTimer::singleShot( 0, [this]() {
        auto total_v = splitter_TopBot->height();
        if ( total_v > 0 ) {
            splitter_TopBot->setSizes( QList<int>() << (total_v * 60 / 100)
                                                    << (total_v * 40 / 100) );
        } else {
            splitter_TopBot->setSizes( QList<int>() << 400 << 250 );
        }

        auto total_h = splitter_SessionAndTabs->width();
        if ( total_h > 0 ) {
            splitter_SessionAndTabs->setSizes( QList<int>() << (total_h * 75 / 100)
                                                            << (total_h * 25 / 100) );
        } else {
            splitter_SessionAndTabs->setSizes( QList<int>() << 900 << 300 );
        }
    } );
