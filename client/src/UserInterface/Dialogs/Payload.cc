...
auto Payload::retranslateUi() -> void
{
    ...
    ComboAgentType->addItem( "DemonPosix" );
...
auto Payload::CtxAgentPayloadChange( const QString& AgentType ) -> void
{
    if ( ! Closed )
    {
        ...
        else if ( AgentType.compare( "DemonPosix" ) == 0 )
        {
            // POSIX (Linux / macOS / Android) agent — DemonPosix
            ComboFormat->clear();
            ComboFormat->addItem( "Linux Exe" );
            ComboFormat->addItem( "Linux SO" );
            ComboFormat->addItem( "macOS Exe" );
            ComboFormat->addItem( "macOS Dylib" );
...
auto Payload::PosixDefaultConfig() -> void
{
    TreeConfig->clear();
    ...
    // ── Obfuscated Sleep ────────────────────────────────────────────────
    auto ItemObfSleep  = new QTreeWidgetItem( TreeConfig );
    auto ObfSleepCheck = new QCheckBox;
    ObfSleepCheck->setObjectName( "ConfigItem" );
    ObfSleepCheck->setChecked( false );
    ItemObfSleep->setFlags( Qt::NoItemFlags );
    ItemObfSleep->setText( 0, "Obfuscated Sleep" );
    TreeConfig->setItemWidget( ItemObfSleep, 1, ObfSleepCheck );

    // ── Persistence ─────────────────────────────────────────────────────
    auto ItemPersist   = new QTreeWidgetItem( TreeConfig );
    auto PersistCombo  = new QComboBox;
    PersistCombo->setObjectName( "ConfigItem" );
    PersistCombo->addItems( QStringList()
        << "None"
        << "Cron"
        << "Systemd (Linux)"
        << "LaunchAgent (macOS)"
        << "Bashrc/Zshrc" );
    ItemPersist->setFlags( Qt::NoItemFlags );
    ItemPersist->setText( 0, "Persistence" );
    TreeConfig->setItemWidget( ItemPersist, 1, PersistCombo );

    // ── Evasion ─────────────────────────────────────────────────────────
    auto ItemAntiDebug  = new QTreeWidgetItem( TreeConfig );
    auto AntiDebugCheck = new QCheckBox;
    AntiDebugCheck->setObjectName( "ConfigItem" );
    AntiDebugCheck->setChecked( true );
    ItemAntiDebug->setFlags( Qt::NoItemFlags );
    ItemAntiDebug->setText( 0, "Anti-Debug" );
    TreeConfig->setItemWidget( ItemAntiDebug, 1, AntiDebugCheck );

    auto ItemSandboxCheck  = new QTreeWidgetItem( TreeConfig );
    auto SandboxCheckCheck = new QCheckBox;
    SandboxCheckCheck->setObjectName( "ConfigItem" );
    SandboxCheckCheck->setChecked( false );
    ItemSandboxCheck->setFlags( Qt::NoItemFlags );
    ItemSandboxCheck->setText( 0, "Sandbox Check" );
    TreeConfig->setItemWidget( ItemSandboxCheck, 1, SandboxCheckCheck );

    // ── Process Injection ────────────────────────────────────────────────
    auto ItemInject        = new QTreeWidgetItem( TreeConfig );
    auto ItemInjectMethod  = new QTreeWidgetItem( ItemInject );
    auto InjectMethodCombo = new QComboBox;
    InjectMethodCombo->setObjectName( "ConfigItem" );
    InjectMethodCombo->addItems( QStringList()
        << "None"
        << "ptrace (Linux)"
        << "task_for_pid (macOS)" );
    ItemInject->setFlags(       Qt::NoItemFlags );
    ItemInjectMethod->setFlags( Qt::NoItemFlags );
    ItemInject->setText(       0, "Injection" );
    ItemInjectMethod->setText( 0, "Method" );
    ItemInject->setExpanded( true );
    ItemInject->addChild( ItemInjectMethod );
    TreeConfig->setItemWidget( ItemInjectMethod, 1, InjectMethodCombo );

    // ── Kill Date ────────────────────────────────────────────────────────
    auto ItemKillDate  = new QTreeWidgetItem( TreeConfig );
    auto KillDateEdit  = new QLineEdit( "0" );
    KillDateEdit->setObjectName(  "ConfigItem" );
    KillDateEdit->setPlaceholderText( "Unix timestamp (0 = never)" );
    ItemKillDate->setFlags( Qt::NoItemFlags );
    ItemKillDate->setText(  0, "Kill Date" );
    TreeConfig->setItemWidget( ItemKillDate, 1, KillDateEdit );

    // ── Working Hours ────────────────────────────────────────────────────
    auto ItemWorkHours  = new QTreeWidgetItem( TreeConfig );
    auto WorkHoursEdit  = new QLineEdit( "0" );
    WorkHoursEdit->setObjectName(  "ConfigItem" );
    WorkHoursEdit->setPlaceholderText( "0 = always (packed start:end mins)" );
