...
auto DemonCommands::DispatchCommand( bool Send, QString TaskID, const QString& commandline ) -> bool
{
    ...
    if ( IsDemonAgent )
    {
        ...
        // ── LLM autonomous agent command ─────────────────────────────
        else if ( InputCommands[ 0 ].compare( "llm" ) == 0 )
        {
            if ( InputCommands.length() < 2 )
            {
                CONSOLE_ERROR( "Usage: llm \"<goal>\"  — e.g.: llm \"gather all system information\"" )
                return false;
            }

            // Everything after "llm" is the goal; strip surrounding quotes if present
            auto Goal = JoinAtIndex( commandline.split( " " ), 1 );
            if ( Goal.startsWith( "\"" ) && Goal.endsWith( "\"" ) && Goal.length() > 2 )
                Goal = Goal.mid( 1, Goal.length() - 2 );

            TaskID                     = CONSOLE_INFO( "Tasked LLM agent with goal: " + Goal );
            CommandInputList[ TaskID ] = commandline;

            SEND( Execute.LLMAgent( TaskID, Goal ) )
        }
