...
func RegisterInfoToInstance(Header Header, RegisterInfo map[string]any) *Agent {
	...
	if val, ok := RegisterInfo["OS Version"]; ok {
	    ...
	    agent.Info.OSVersion = getOsVersionString(OsVersion)
	}
	...
	*/
	...
	if Parser.Length() >= 32+16 {
		...
		if Parser.CanIRead([]parser.ReadType{parser.ReadInt32, parser.ReadBytes, parser.ReadBytes, parser.ReadBytes, parser.ReadBytes, parser.ReadBytes, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt32, parser.ReadInt64, parser.ReadInt32}) {
			...
			Session.Info.OSVersion = getOsVersionString(OsVersion)
...
// DEMON_OS_* constants — must match DemonPosix.h and DemonAndroid
const (
	DemonOsWindows = 0x01
	DemonOsLinux   = 0x02
	DemonOsMacOS   = 0x03
...
// getOsVersionString returns a human-readable OS version string.
// OsVersion is a 5-element slice:
//   [0] major  [1] minor  [2] workstation(1)/server(0) or platform type
//   [3] service-pack / 0   [4] build-number / platform-type
//
// For POSIX agents the 5th slot ([4]) carries DEMON_OS_LINUX or
// DEMON_OS_MACOS so we can distinguish them from Windows agents.
func getOsVersionString(OsVersion []int) string {
	if len(OsVersion) < 5 {
		return "Unknown"
	}

	// Check POSIX platform type stored in slot [4]
	switch OsVersion[4] {
	case DemonOsLinux:
		return fmt.Sprintf("Linux %d.%d.%d", OsVersion[0], OsVersion[1], OsVersion[2])
	case DemonOsMacOS:
		return getMacOsVersionString(OsVersion[0], OsVersion[1], OsVersion[2])
	case DemonOsAndroid:
		return fmt.Sprintf("Android %d (API %d)", OsVersion[0], OsVersion[1])
	}

	// Windows path (legacy — unchanged)
	return getWindowsVersionString(OsVersion)
}

// getMacOsVersionString maps macOS kernel version to marketing name.
func getMacOsVersionString(major, minor, patch int) string {
	// macOS uses Darwin (XNU) kernel versions — uname returns those.
	// Common mappings:
	switch {
	case major >= 24:
		return fmt.Sprintf("macOS Sequoia 15 (%d.%d.%d)", major, minor, patch)
	case major == 23:
		return fmt.Sprintf("macOS Sonoma 14 (%d.%d.%d)", major, minor, patch)
	case major == 22:
		return fmt.Sprintf("macOS Ventura 13 (%d.%d.%d)", major, minor, patch)
	case major == 21:
		return fmt.Sprintf("macOS Monterey 12 (%d.%d.%d)", major, minor, patch)
	case major == 20:
		return fmt.Sprintf("macOS Big Sur 11 (%d.%d.%d)", major, minor, patch)
	case major == 19:
		return fmt.Sprintf("macOS Catalina 10.15 (%d.%d.%d)", major, minor, patch)
	case major == 18:
		return fmt.Sprintf("macOS Mojave 10.14 (%d.%d.%d)", major, minor, patch)
	case major == 17:
		return fmt.Sprintf("macOS High Sierra 10.13 (%d.%d.%d)", major, minor, patch)
	default:
		return fmt.Sprintf("macOS %d.%d.%d", major, minor, patch)
	}
}
