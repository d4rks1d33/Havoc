...
func (h *HTTP) request(ctx *gin.Context) {
        ...
        // Transparently unwrap JSON-wrapped agent requests.
        jsonWrapped := false
        if len(Body) > 0 && Body[0] == '{' {
                if unwrapped := unwrapAgentJSON(Body); len(unwrapped) > 0 {
                        Body = unwrapped
                        jsonWrapped = true
                }
        }
        ...
        */
        ...
        if Response, Success := parseAgentRequest(h.Teamserver, Body, ExternalIP); Success {
                ...
                if jsonWrapped {
                        // JSON-wrapped request → JSON-wrapped response (POSIX agent evasion)
                        wrapped := wrapAgentJSON(respBytes)
                        ctx.Header("Content-Type", "application/json")
                        _, err := ctx.Writer.Write(wrapped)
                        if err != nil {
                        ...
                        // Raw binary request → raw binary response (Windows Demon)
                        _, err := ctx.Writer.Write(respBytes)
                        if err != nil {
                                logger.Debug("Failed to write to request: " + err.Error())
                                h.fake404(ctx)
                                return
...
// ── JSON envelope helpers (for POSIX agent evasion) ───────────────────

// unwrapAgentJSON decodes {"d":"<base64>","t":<timestamp>} from POSIX agents.
// Returns the raw binary payload, or nil if the body is not a wrapped packet.
func unwrapAgentJSON(body []byte) []byte {
        ...
        // raw is a JSON string — unmarshal to get the base64 string value
        var b64 string
...
// wrapAgentJSON encodes a binary response as {"d":"<base64>"} so the
// POSIX agent can unwrap it client-side.
func wrapAgentJSON(data []byte) []byte {
