...
func (t *Teamserver) AgentConsole(AgentID string, CommandID int, Output map[string]string) {
	...
	// If there's an LLM agent waiting for output from this agent,
        // forward the output text to it (keyed by AgentID).
        if ch, loaded := t.LLMPending.Load(AgentID); loaded {
                text := Output["Output"]
                if msg, ok2 := Output["Message"]; ok2 && text == "" {
                        text = msg
                }
                if text != "" {
                        select {
                        case ch.(chan string) <- text:
                        default:
                        }
                }
        }
